...
func formNewRoundKey(chainType account.ChainType, ledgerHeight int64) []byte {
	return formKey(string(BlockKey), chainType.String(), fmt.Sprintf("%d", ledgerHeight))
}

func (storage *Storage) RoundHeight(chainType account.ChainType, ledgerHeight int64) (uint64, error) {
...
func (storage *Storage) SetNewRound(chainType account.ChainType, ledgerHeight int64, tcHeight uint64) error {
	var b [8]byte
