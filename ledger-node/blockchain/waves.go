...
func (waves *Waves) SendOraclesToNebula(nebulaId account.NebulaId, oracles []account.OraclesPubKey, signs [][]byte, round int64, ctx context.Context) (string, error) {
	...
	lastRoundState, err := waves.helper.GetStateByAddressAndKey(nebulaAddress, "last_round_"+fmt.Sprintf("%d", round))
	if err != nil {
		return "", err
	}
	...
	oracleCountState, err := waves.helper.GetStateByAddressAndKey(nebulaAddress, "oracle_count")
	if err != nil {
		return "", err
	}
...
func (waves *Waves) SendConsulsToGravityContract(newConsulsAddresses []account.OraclesPubKey, signs [][]byte, round int64, ctx context.Context) (string, error) {
	state, err := waves.helper.GetStateByAddressAndKey(waves.gravityContractAddress, "last_round_"+fmt.Sprintf("%d", round))
	if err != nil {
		return "", err
	}
	if state != nil {
	...
	consulsCountState, err := waves.helper.GetStateByAddressAndKey(waves.gravityContractAddress, "consuls_count")
	if err != nil {
		return "", err
	}
	...
	err = <-waves.helper.WaitTx(tx.ID.String())
	if err != nil {
		return "", err
	}
...
func (waves *Waves) signOracles(consulsAddresses []account.OraclesPubKey) ([]byte, error) {
	...
	sign, err := account.SignWithTC(waves.privKeyBytes, []byte(strings.Join(stringOracles, ",")), account.Waves)
	if err != nil {
		return nil, err
	}
