...
func initLedgerConfig(ctx *cli.Context) error {
	...
	/*
		rpcConfig, err := rpc.NewRPCConfig(rpcHost, cfg.RPCHost, validatorPrivKey)
		if err != nil {
			return err
		}
...
func startLedger(ctx *cli.Context) error {
	...
	rpc := ctx.String(PrivateRPCHostFlag)
	...
	app, err := crateApp(db, privKeysCfg, ledgerConf, genesis, bootstrap, sysCtx)
	if err != nil {
...
func crateApp(db *badger.DB, privKeys config.PrivKeys, cfg config.LedgerConfig, genesisCfg config.Genesis, bootstrap string, ctx context.Context) (*app.GHApplication, error) {
	var ledgerPrivKey ed25519.PrivKeyEd25519
	ledgerPrivKeyBytes, err := hexutil.Decode(privKeys.Validator)
	if err != nil {
		return nil, err
	}
	...
	for k, v := range cfg.Adapters {
		...
		privKey, err := account.StringToPrivKey(privKeys.TargetChains[k], chainType)
		if err != nil {
			return nil, err
		}
