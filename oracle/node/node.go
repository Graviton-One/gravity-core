...
func New(nebulaId account.NebulaId, chainType account.ChainType,
	...
	zap.L().Sugar().Debugf("Creating adaptor ChType: %s, targetChainNode: %s", chType, targetChainNodeUrl)
	...
	zap.L().Sugar().Debugf("NebulaId %s", nebulaId.ToString(account.ChainType(ctype)))
